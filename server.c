...
int main() {
  ...
  while (1) {
    ...
    pthread_create(&tid[client_counter], NULL, handle_client,
                   (void *)&clients[client_counter]);
...
void *handle_client(void *arg) {
  ...
  while (1) {
    ...
    if (send(client_socket, "Messaggio ricevuto dal server",
             strlen("Messaggio ricevuto dal server"), 0) == -1) {
