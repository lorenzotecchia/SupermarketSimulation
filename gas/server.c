...
// Dichiarazione della funzione lunghezzaCoda
int lunghezzaCoda(Queue *coda);
...
Server *creaServer() {
    ...
    for (int i = 0; i < NUM_CASSE; i++) {
        ...
        pthread_create(&server->threads[i], NULL, gestisciCassa, (void *)server->casse[i]);
    }
...
void accettaClient(Server *server) {
    ...
    while (1) {
        ...
        *client_socket = accept(server->server_fd, (struct sockaddr *)&address, (socklen_t*)&addrlen);
        if (*client_socket < 0) {
            perror("accept");
            free(client_socket);
            continue;
